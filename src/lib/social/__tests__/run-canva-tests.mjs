 ...
 *
 *   node \
 *     --require "C:/Users/Nazre/centinelia/node_modules/jiti/lib/jiti-native.cjs" \
 *     src/lib/social/__tests__/run-canva-tests.mjs
 *
 * Or via the centinelia node_modules jiti binary:
 *
 *   "C:/Users/Nazre/centinelia/node_modules/.bin/jiti" \
 *     src/lib/social/__tests__/run-canva-tests.mjs
 *
...
// via `node --require jiti/...`, TypeScript files are already resolvable.
...
try {
  ...
  console.error('');
  console.error('  node --require "C:/Users/Nazre/centinelia/node_modules/jiti/lib/jiti-native.cjs" \\');
  ...
  test('throws CanvaApiError when API returns non-2xx', async () => {
    ...
    try {
      ...
      await assert.rejects(
        () => provider.listBrandTemplates('bad-token'),
        (err) => {
...
describe('CanvaProvider rate limit backoff', () => {
  ...
  test('throws after exhausting max retries (5) on persistent 429', async () => {
    ...
    try {
      ...
      await assert.rejects(
        () => provider.listBrandTemplates(ACCESS_TOKEN),
        (err) => {
          assert.ok(
...
describe('CanvaProvider.exportDesign', () => {
  test('creates export job, polls until success, returns export URL', async () => {
    let callCount = 0;
    const original = globalThis.fetch;
    globalThis.fetch = async () => {
      callCount++;
...
describe('CanvaProvider.uploadAsset', () => {
  test('uploads file buffer and returns assetId', async () => {
    const original = globalThis.fetch;
    globalThis.fetch = async () =>
      makeFetchResponse({
        asset: { id: 'assetUploaded001', name: 'logo.png', created_at: 1726030000, updated_at: 1726030001 },
      });
    try {
      const provider = new CanvaProvider({ clientId: CLIENT_ID, clientSecret: CLIENT_SECRET });
