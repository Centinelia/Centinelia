...
function buildToolDef(name: string, agent: VoiceAgent, server: ServerFn): ToolDef | null {
  switch (name) {
    ...
    case 'crear_lead': return { type: 'function', function: { name: 'crear_lead', description: 'Registra los datos de un prospecto interesado en contratar servicios. Si ya lo registraste en los últimos minutos, el sistema hace merge automático — NO re-ejecutes la tool para "confirmar", solo dile al cliente dónde revisar (sección Llamadas del portal).', parameters: { type: 'object', properties: { nombre: { type: 'string', description: 'Nombre completo del prospecto' }, negocio: { type: 'string', description: 'Nombre del negocio' }, giro: { type: 'string', description: 'Giro o industria del negocio' }, servicio: { type: 'string', description: 'Servicio que necesita' }, presupuesto: { type: 'string', description: 'Presupuesto aproximado' }, timeline: { type: 'string', description: 'Para cuándo lo necesita' }, email: { type: 'string', description: 'Correo electrónico' }, whatsapp: { type: 'string', description: 'Número de WhatsApp' } }, required: ['nombre', 'servicio'] } }, server: server('crear-lead') };

    case 'crear_contacto_saliente': return { type: 'function', function: { name: 'crear_contacto_saliente', description: 'Agrega un prospecto a la lista de contactos salientes para llamarle después (aparece en la sección Campañas del portal). Úsala cuando el cliente pida seguimiento o cuando registres un lead que debe recibir llamada de vuelta. Es adicional a crear_lead — puedes llamar ambas cuando aplica.', parameters: { type: 'object', properties: { nombre: { type: 'string', description: 'Nombre del contacto' }, telefono: { type: 'string', description: 'Teléfono a llamar (obligatorio)' }, motivo: { type: 'string', description: 'Motivo o contexto de la llamada de seguimiento' }, scheduled_at: { type: 'string', description: 'Fecha/hora ISO 8601 sugerida para la llamada (opcional). Si se omite, queda en pending sin agendar.' } }, required: ['telefono'] } }, server: server('crear-contacto-saliente') };
