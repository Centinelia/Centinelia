...
function buildToolDef(name: string, agent: VoiceAgent, server: ServerFn): ToolDef | null {
  switch (name) {
    ...
    case 'agendar_cita': return { type: 'function', function: { name: 'agendar_cita', description: 'Agenda, modifica o cancela una cita. IMPORTANTE: siempre proporciona fecha_iso (YYYY-MM-DD) y hora (HH:MM 24h) para que el sistema pueda detectar empalmes y sincronizar con el calendario conectado.', parameters: { type: 'object', properties: { accion: { type: 'string', enum: ['agendar', 'modificar', 'cancelar'], description: 'Acción a realizar' }, nombre: { type: 'string', description: 'Nombre del cliente' }, servicio: { type: 'string', description: 'Servicio para la cita' }, fecha: { type: 'string', description: 'Fecha en lenguaje natural para mostrar al cliente (ej: lunes 23 de junio)' }, fecha_iso: { type: 'string', description: 'Fecha ISO YYYY-MM-DD (ej: 2026-08-07). Obligatoria para agendar/modificar: sin esto el sistema no puede detectar empalmes.' }, hora: { type: 'string', description: 'Hora en formato HH:MM 24h (ej: 14:30 para 2:30pm). Obligatoria para agendar/modificar.' }, duracion_min: { type: 'number', description: 'Duracion estimada de la cita en minutos. Default 60.' }, telefono: { type: 'string', description: 'Teléfono de confirmación' } }, required: ['accion', 'nombre'] } }, server: server('agendar-cita') };
