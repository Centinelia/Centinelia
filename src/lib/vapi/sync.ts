...
function buildToolDef(name: string, agent: VoiceAgent, server: ServerFn): ToolDef | null {
  switch (name) {
    ...
    case 'list_calendar_events': return { type: 'function', function: { name: 'list_calendar_events', description: 'Consulta los eventos del calendario del dueño en un rango de fechas.', parameters: { type: 'object', properties: { from: { type: 'string', description: 'Fecha/hora ISO 8601 de inicio del rango (ej: 2026-08-11T00:00:00-06:00)' }, to: { type: 'string', description: 'Fecha/hora ISO 8601 de fin del rango (ej: 2026-08-12T00:00:00-06:00)' } }, required: ['from', 'to'] } }, server: server('exec/list_calendar_events') };

    case 'create_calendar_event': return { type: 'function', function: { name: 'create_calendar_event', description: 'Crea un evento en el calendario del dueño. start/end en ISO 8601 con timezone. Puede generar link de Google Meet automáticamente — cuando el cliente pida videollamada NO pidas link, deja que el sistema lo cree (generate_meet_link=true). El link vuelve en el message del tool_result.', parameters: { type: 'object', properties: { title: { type: 'string', description: 'Título del evento' }, start: { type: 'string', description: 'Inicio en ISO 8601 con timezone (ej: 2026-08-11T12:00:00-06:00)' }, end: { type: 'string', description: 'Fin en ISO 8601 con timezone (ej: 2026-08-11T13:00:00-06:00)' }, description: { type: 'string', description: 'Descripción o notas del evento (opcional)' }, location: { type: 'string', description: 'Ubicación o link de videollamada (opcional; si dices "Google Meet" sin link, se auto-genera)' }, attendees: { type: 'array', items: { type: 'string' }, description: 'Correos de invitados (opcional)' }, generate_meet_link: { type: 'boolean', description: 'true para generar link Meet automáticamente. Úsalo cuando el cliente pida videollamada o Meet y NO tengas un link propio.' } }, required: ['title', 'start', 'end'] } }, server: server('exec/create_calendar_event') };

    case 'delete_calendar_event': return { type: 'function', function: { name: 'delete_calendar_event', description: 'Elimina o cancela un evento del calendario del dueño.', parameters: { type: 'object', properties: { event_id: { type: 'string', description: 'ID del evento a eliminar (obtenido de list_calendar_events)' } }, required: ['event_id'] } }, server: server('exec/delete_calendar_event') };

    case 'buscar_correo_enviado': return { type: 'function', function: { name: 'buscar_correo_enviado', description: 'Busca correos que TÚ u otro empleado del equipo hayan enviado en el pasado desde el buzón de la empresa. Útil para ver qué se le comunicó a un cliente antes, revisar histórico de conversaciones, o antes de reenviar/dar seguimiento. Devuelve preview del cuerpo + destinatario + asunto + fecha + qué empleado lo envió.', parameters: { type: 'object', properties: { query: { type: 'string', description: 'Texto a buscar en asunto o cuerpo (opcional). Ej: "cotización Nemak", "confirmación cita".' }, destinatario: { type: 'string', description: 'Correo del destinatario para filtrar (opcional, match parcial). Ej: "pedro" o "@nemak.com".' }, dias: { type: 'number', description: 'Días hacia atrás a buscar (1-365, default 30).' }, limit: { type: 'number', description: 'Máximo resultados a devolver (1-20, default 10).' } } } }, server: server('exec/buscar_correo_enviado') };
