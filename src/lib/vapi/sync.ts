...
function buildToolDef(name: string, agent: VoiceAgent, server: ServerFn): ToolDef | null {
  switch (name) {
    ...
    case 'create_calendar_event': return { type: 'function', function: { name: 'create_calendar_event', description: 'Crea un evento en el calendario del dueño. start/end en ISO 8601 con timezone.', parameters: { type: 'object', properties: { title: { type: 'string', description: 'Título del evento' }, start: { type: 'string', description: 'Inicio en ISO 8601 con timezone (ej: 2026-08-11T12:00:00-06:00)' }, end: { type: 'string', description: 'Fin en ISO 8601 con timezone (ej: 2026-08-11T13:00:00-06:00)' }, description: { type: 'string', description: 'Descripción o notas del evento (opcional)' }, location: { type: 'string', description: 'Ubicación o link de videollamada (opcional)' }, attendees: { type: 'array', items: { type: 'string' }, description: 'Correos de invitados (opcional)' } }, required: ['title', 'start', 'end'] } }, server: server('exec/create_calendar_event') };
