...
function buildToolDef(name: string, agent: VoiceAgent, server: ServerFn): ToolDef | null {
  switch (name) {
    ...
    case 'trigger_outbound_call': return { type: 'function', function: { name: 'trigger_outbound_call', description: 'Dispara una llamada saliente a un número específico. Úsala para escalar a otro contacto (encargado de operaciones, dispatcher, gerente) cuando el cliente reporta un problema que requiere acción del equipo del negocio. Después de disparar, avísale al cliente que ya estás notificando y cierra la llamada. Requiere motivo detallado que describa el problema: quién lo reporta, qué pasó, dónde, cuándo — para que el destinatario pueda actuar sin preguntar contexto.', parameters: { type: 'object', properties: { phone_number: { type: 'string', description: 'Teléfono E.164 del destinatario. Ej: +528112803360.' }, contact_name: { type: 'string', description: 'Nombre del destinatario (opcional, mejora la personalización del saludo).' }, message: { type: 'string', description: 'Motivo detallado que el empleado leerá al destinatario. Incluye nombre del cliente afectado, dirección o zona, producto/servicio, fecha aproximada y qué reportó exactamente.' } }, required: ['phone_number', 'message'] } }, server: server('exec/trigger_outbound_call') };
