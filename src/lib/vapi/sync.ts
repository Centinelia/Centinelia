...
function buildToolDef(name: string, agent: VoiceAgent, server: ServerFn): ToolDef | null {
  switch (name) {
    ...
    case 'buscar_documento_oficina': return { type: 'function', function: { name: 'buscar_documento_oficina', description: 'Busca documentos ya generados y guardados en la Oficina del negocio (facturas, cotizaciones, cartas, propuestas, órdenes). Úsala cuando el cliente pida "el documento que le mandé la semana pasada" o cuando quieras reutilizar algo antes de generar nuevo. Devuelve una lista con id, título, tipo, cliente y fecha — luego usa enviar_documento_oficina con el id para adjuntarlo a un correo.', parameters: { type: 'object', properties: { query: { type: 'string', description: 'Texto para buscar en título, filename o nombre del cliente. Opcional.' }, kind: { type: 'string', description: 'Filtro por tipo exacto: factura, cotizacion, orden_compra, proposal, letter, general, nota_venta, excel, word, powerpoint.' }, cliente: { type: 'string', description: 'Filtro por nombre del cliente (fuzzy).' }, limit: { type: 'number', description: 'Máximo de resultados. Default 10, máximo 50.' } }, required: [] } }, server: server('buscar-documento-oficina') };
