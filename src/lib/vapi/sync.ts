...
function buildToolDef(name: string, agent: VoiceAgent, server: ServerFn): ToolDef | null {
  switch (name) {
    ...
    // create_contract_draft — schema CRITICAL alineado con chat/email + handler.
    // Voice antes usaba {tipo, cliente_nombre, cliente_rfc, descripcion, monto,
    // vigencia, notas} — cero overlap con el shape del handler (executor.ts:204)
