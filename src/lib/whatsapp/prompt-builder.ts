...
Usa la herramienta guardar_lead con el campo servicio describiendo el pedido.`);
  ...
  // Kill switch (Fase 9.2): si agent_missions_enabled=false, omitir bloque.
  {
    ...
    const waFeaturesForFlag = agent.org_features ?? (agent.features as unknown as Record<string, unknown> | null) ?? null;
  ...
  // Kill switch (Fase 9.2): si agent_missions_enabled=false, omitir bloque.
  {
    ...
    const waFeaturesForTasks = agent.org_features ?? (agent.features as unknown as Record<string, unknown> | null) ?? null;
