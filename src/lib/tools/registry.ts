...
const TOOL_REGISTRY_BASE: Omit<ToolEntry, 'pack'>[] = [
  ...
  { name: 'pedir_datos_faltantes',    description: 'Marca que el correo requiere más datos del cliente para poder timbrar. Nala redacta la respuesta pidiéndolos.', channels: A, category: 'fiscal', destructive: false, gatedByRole: ['nala'], gatedByFeature: null, capability: null, policy: DEFAULT_POLICY },
  { name: 'reportar_bug_a_nash',      description: 'Reporta un bug/limitación del sistema a Nash (crea platform_incident). Úsala cuando el problema NO es del cliente sino del código Centinelia.', channels: A, category: 'fiscal', destructive: false, gatedByRole: ['nala'], gatedByFeature: null, capability: null, policy: DEFAULT_POLICY },
  { name: 'registrar_pago_pendiente_verificacion', description: 'Recibe comprobante SPEI del cliente. Evalúa reglas de auto-aprobación (UUID válido + cliente activo + monto exacto + no dedupe). Si todas cumplen, timbra REP inmediato. Si algo falla, guarda pending + notifica a Nazre.', channels: A, category: 'fiscal', destructive: true, gatedByRole: ['nala'], gatedByFeature: null, capability: null, policy: DEFAULT_POLICY },
