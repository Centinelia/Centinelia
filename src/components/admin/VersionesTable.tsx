      ...
      >
        <table className="w-full text-[13px]">
          <thead style={{ background: '#FAFAFB' }}>
            <tr>
              <th className="text-left px-4 py-3 text-[11px] uppercase tracking-wider font-medium" style={{ color: '#6B6480' }}>Meerkat</th>
              <th className="text-left px-4 py-3 text-[11px] uppercase tracking-wider font-medium" style={{ color: '#6B6480' }}>Rollout activo</th>
              <th className="text-left px-4 py-3 text-[11px] uppercase tracking-wider font-medium" style={{ color: '#6B6480' }}>Fallback</th>
              <th className="text-left px-4 py-3 text-[11px] uppercase tracking-wider font-medium" style={{ color: '#6B6480' }}>Última activación</th>
              <th className="text-left px-4 py-3 text-[11px] uppercase tracking-wider font-medium" style={{ color: '#6B6480' }}>Agentes</th>
              <th className="text-left px-4 py-3 text-[11px] uppercase tracking-wider font-medium" style={{ color: '#6B6480' }}>Disponibles</th>
              <th className="text-right px-4 py-3 text-[11px] uppercase tracking-wider font-medium" style={{ color: '#6B6480' }}>Acciones</th>
            </tr>
          </thead>
          <tbody>
            {rows.map((r, i) => (
              ...
              >
                <td className="px-4 py-2.5 font-medium" style={{ color: '#1A0A3B' }}>{r.meerkat_id}</td>
                <td className="px-4 py-2.5">
                  {r.rollouts.length === 0 ? (
                    <span className="text-[12px]" style={{ color: '#9B8FB5' }}>sin flags</span>
                  ) : (
                        ...
                        const style = ro.killed
                          ...
                          : ro.pct === 100
                            ...
                            : ro.pct > 0
                              ...
                              : { background: '#F5F0FF', color: '#6B6480', border: '1px solid #E8E3F5' };
                        return (
                          <a
                            key={ro.version}
                ...
                <td className="px-4 py-2.5">
                  <span
                    ...
                    style={{ background: '#F5F0FF', color: '#4A3B6B', border: '1px solid #E8E3F5' }}
                  >
                ...
                <td className="px-4 py-2.5 text-[12px]" style={{ color: '#6B6480' }}>
                  ...
                  {r.activated_by && <span style={{ color: '#9B8FB5' }}> · {r.activated_by}</span>}
                </td>
                <td className="px-4 py-2.5 tabular-nums" style={{ color: '#1A0A3B' }}>
                  {r.agent_count}
                  {r.pinned_count > 0 && (
                    <span className="text-[12px]" style={{ color: '#9B8FB5' }}> ({r.agent_count - r.pinned_count} latest, {r.pinned_count} pinned)</span>
                  )}
                </td>
                ...
                <td className="px-4 py-2.5 text-right whitespace-nowrap">
                  {r.available_versions.length > 1 && (
                    <button
                      ...
                      style={{ border: '1px solid #E8E3F5', color: '#4A3B6B', background: '#FFFFFF' }}
                    >
