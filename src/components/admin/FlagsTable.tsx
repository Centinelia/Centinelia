      ...
      <div className="flex flex-wrap items-center gap-2">
        {(['all', 'meerkat', 'portal', 'tool', 'silent'] as Prefix[]).map(p => (
          <button
            ...
            style={{
              ...
              color:      prefix === p ? '#6C3BFF' : '#4A3B6B',
              border:     `1px solid ${prefix === p ? '#DDD6FE' : '#E8E3F5'}`,
            }}
          >
            {p === 'all' ? 'Todos' : p}
          </button>
        ...
        <button
          ...
          style={{
            ...
            color:      onlyKilled ? '#B91C1C' : '#4A3B6B',
            border:     `1px solid ${onlyKilled ? '#FECACA' : '#E8E3F5'}`,
          }}
        >
          <Ban size={13} />
          Solo killed
      ...
      >
        <table className="w-full text-[13px]">
          <thead style={{ background: '#FAFAFB' }}>
            <tr>
              <th className="text-left px-4 py-3 text-[11px] uppercase tracking-wider font-medium" style={{ color: '#6B6480' }}>flag_key</th>
              <th className="text-right px-4 py-3 text-[11px] uppercase tracking-wider font-medium" style={{ color: '#6B6480' }}>%</th>
              <th className="text-right px-4 py-3 text-[11px] uppercase tracking-wider font-medium" style={{ color: '#6B6480' }}>allow</th>
              <th className="text-right px-4 py-3 text-[11px] uppercase tracking-wider font-medium" style={{ color: '#6B6480' }}>deny</th>
              <th className="text-center px-4 py-3 text-[11px] uppercase tracking-wider font-medium" style={{ color: '#6B6480' }}>Estado</th>
              <th className="text-right px-4 py-3 text-[11px] uppercase tracking-wider font-medium" style={{ color: '#6B6480' }}>Actualizado</th>
            </tr>
          </thead>
          <tbody>
            {filtered.length === 0 && (
              <tr>
                <td colSpan={6} className="text-center px-4 py-10 text-[13px]" style={{ color: '#6B6480' }}>
            ...
            {filtered.map((f, i) => {
              ...
              return (
                ...
                >
                  ...
                  <td className="px-4 py-2.5 text-right font-mono tabular-nums" style={{ color: '#1A0A3B' }}>{f.rollout_pct}</td>
                  <td className="px-4 py-2.5 text-right font-mono tabular-nums" style={{ color: '#6B6480' }}>{f.allowlist.length}</td>
                  <td className="px-4 py-2.5 text-right font-mono tabular-nums" style={{ color: '#6B6480' }}>{f.denylist.length}</td>
                  ...
                  <td className="px-4 py-2.5 text-right text-[12px]" style={{ color: '#6B6480' }}>
