...
export default async function HelpdeskPage({ params }: Props) {
  ...
  // Detect IT sub-user: logged-in as sub-user with of_helpdesk module
  const isItSubUser = !!(session?.isSubUser && session.modules?.includes('of_helpdesk'));

  // Look up sub-user display name (used to filter assigned tickets)
  ...
  // Check if any agent in the account is Neo (meerkat_role_id = 'neo')
  let hasNeo = false;
