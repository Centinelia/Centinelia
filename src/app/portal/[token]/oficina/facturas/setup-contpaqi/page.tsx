...
import { ArrowLeft, CheckCircle2, AlertCircle, Loader2, Download, Copy, ExternalLink, KeyRound, Info } from 'lucide-react';
...
// Régimenes fiscales SAT más comunes.
    ...
    <div className="p-6 max-w-4xl mx-auto">
      ...
      <header className="mb-6">
        ...
        <p className="text-xs mt-1" style={{ color: 'var(--c-text-3)' }}>
          Configura tus datos fiscales y descarga el instalador del Writer para Windows. El Writer sincroniza tu catálogo de CONTPAQi con Dropbox y timbra las facturas que Nala genere.
        </p>
      </header>

      {loading && <Spinner label="Cargando estado..." />}

      {!loading && !state?.dropbox_connected && (
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              <p className="text-sm font-semibold" style={{ color: '#b45309' }}>
                Primero conecta Dropbox
              </p>
              <p className="text-xs mt-1" style={{ color: '#92400e' }}>
                El Writer sincroniza tu catálogo de CONTPAQi con Dropbox. Necesitas tener Dropbox conectado en el empleado Nala antes de configurar esto.
              </p>
              <p className="text-xs mt-2">
                Ve a <strong>Empleados → Nala → Configurar → Herramientas → Almacenamiento en la nube → Conectar Dropbox</strong>.
              </p>
            </div>
          </div>
        </div>
      ...
      {!loading && state?.dropbox_connected && (
        <>
          {/* Dropbox status */}
          <div className="rounded-lg px-4 py-2.5 mb-5 flex items-center gap-2 text-xs"
          ...
          {/* Form */}
          <form onSubmit={submit} className="rounded-2xl p-5 mb-5"
          ...
          {/* Post-save panel: descarga installer + config */}
          {(state?.configured || saved) && state?.writer_api_token && (
            <div className="rounded-2xl p-5 mb-5"
                 style={{ background: 'rgba(108,59,255,0.05)', border: '1px solid rgba(108,59,255,0.2)' }}>
              <h2 className="text-sm font-semibold mb-1 flex items-center gap-1.5" style={{ color: '#6C3BFF' }}>
              ...
              <div className="mt-5">
                <h3 className="text-xs font-semibold uppercase tracking-widest mb-2" style={{ color: 'var(--c-text-2)' }}>
                  Configuración del Writer (pega esto en el primer arranque)
                </h3>
                <CopyableConfig
                  values={{
              ...
              <div className="mt-5 text-xs space-y-1" style={{ color: 'var(--c-text-2)' }}>
                ...
                <ol className="list-decimal pl-5 space-y-0.5">
                  ...
                  <li>Al primer arranque, pega la configuración de arriba cuando lo pida.</li>
                  <li>Conecta a tu CONTPAQi (dirá qué empresa/BD elegir).</li>
  ...
  return (
    <div>
      <button onClick={download} disabled={busy}
        ...
        Descargar Writer para Windows (.zip)
      </button>
