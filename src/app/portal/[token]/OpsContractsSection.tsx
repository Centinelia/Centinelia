...
import { Plus, ChevronDown, ChevronUp, Trash2, FileText, AlertTriangle, Clock, Search } from 'lucide-react';
...
export default function OpsContractsSection({ token }: { token: string }) {
  ...
  async function handleCreate(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      const alertDays = form.alert_days_before
        .split(',')
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {/* Create form */}
      {showForm && (
        <form onSubmit={handleCreate} className="rounded-xl p-4 flex flex-col gap-3"
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="col-span-2">
              ...
              <input required value={form.name} onChange={e => setForm(p => ({ ...p, name: e.target.value }))}
                placeholder="Ej. Arrendamiento local 5"
            ...
            <div>
              <label className="text-xs block mb-1" style={{ color: 'var(--c-text-3)' }}>Alertar (días antes, separados por coma)</label>
              <input value={form.alert_days_before} onChange={e => setForm(p => ({ ...p, alert_days_before: e.target.value }))}
                placeholder="30,7,1"
                className="w-full px-3 py-2 rounded-lg text-sm"
                style={{ background: 'var(--c-bg)', border: '1px solid var(--c-border)', color: 'var(--c-text)' }} />
            </div>
      ...
      <div className="relative">
        ...
        <input
          ...
          placeholder="Buscar por nombre, contraparte o notas..."
          ...
          {filtered.map(c => {
            ...
            return (
              <div key={c.id} className="rounded-xl overflow-hidden"
                style={{ background: 'var(--c-surface)', border: `1px solid ${days >= 0 && days <= 7 && c.status === 'activo' ? 'rgba(239,68,68,0.3)' : days <= 30 && days >= 0 && c.status === 'activo' ? 'rgba(245,158,11,0.25)' : 'var(--c-border)'}` }}>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-1.5 flex-wrap">
                      ...
                      <span className="text-xs px-1.5 py-0.5 rounded-full font-medium"
                        style={{ background: statusCf.bg, color: statusCf.color }}>
                        {statusCf.label}
                      </span>
                    </div>
                    <div className="flex items-center gap-2 mt-0.5">
                      <p className="text-xs flex items-center gap-1" style={{ color: uColor }}>
                        <Clock size={10} />
                        {c.status === 'activo'
                          ? days < 0 ? 'Vencido' : days === 0 ? '¡Vence hoy!' : `Vence en ${days} día${days !== 1 ? 's' : ''}`
                          : new Date(c.expiry_date + 'T12:00:00').toLocaleDateString('es-MX', { day: 'numeric', month: 'short', year: 'numeric' })
                        }
                      </p>
                  ...
                  <div className="px-4 pb-4 flex flex-col gap-3"
                    ...
                    <div className="pt-3 grid grid-cols-2 gap-3">
                      <div>
                        ...
                        <p className="text-sm font-medium" style={{ color: 'var(--c-text)' }}>
                          {new Date(c.expiry_date + 'T12:00:00').toLocaleDateString('es-MX', { day: 'numeric', month: 'long', year: 'numeric' })}
                        </p>
                      </div>
                      <div>
                    ...
                    {c.notes && (
                      <div className="rounded-lg p-3" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid var(--c-border)' }}>
                        <p className="text-xs mb-1" style={{ color: 'var(--c-text-3)' }}>Notas</p>
                        <p className="text-sm" style={{ color: 'var(--c-text)' }}>{c.notes}</p>
                      </div>
                    )}

                    {c.renewal_draft && (
                      <div className="rounded-lg p-3" style={{ background: 'rgba(108,59,255,0.06)', border: '1px solid rgba(108,59,255,0.2)' }}>
                        <p className="text-xs mb-2 font-semibold" style={{ color: '#9B6DFF' }}>Borrador de renovación (generado por IA)</p>
                        <p className="text-xs leading-relaxed whitespace-pre-wrap" style={{ color: 'var(--c-text-2)' }}>{c.renewal_draft}</p>
                      </div>
                    )}

                    <div className="flex items-center gap-2 flex-wrap">
                      {c.status === 'activo' && (
                        <button onClick={() => handleStatus(c.id, 'renovado')}
                          className="px-3 py-1.5 rounded-lg text-xs font-medium transition-opacity hover:opacity-80"
                          style={{ background: 'rgba(155,109,255,0.1)', border: '1px solid rgba(155,109,255,0.25)', color: '#9B6DFF' }}>
                          Marcar renovado
                        </button>
                      )}
                      {(c.status === 'vencido' || c.status === 'renovado') && (
                        <button onClick={() => handleStatus(c.id, 'activo')}
                          className="px-3 py-1.5 rounded-lg text-xs font-medium transition-opacity hover:opacity-80"
