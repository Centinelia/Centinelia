/**
 * POST /api/portal/[token]/social/media/upload
 *
 * Recibe un archivo multimedia (video o imagen) desde el portal del cliente,
 * lo sube al bucket 'user-media' y registra la fila en user_media_uploads.
 *
 * Guardas de seguridad (espejo de upload-logo):
 *   - Session cookie obligatoria (verifySession)
 *   - Resolución de org desde token (resolveOrgFromToken)
 *   - IDOR: agent_id debe pertenecer al mismo portal_email que la sesión
 *
 * Límites de tamaño: 100 MB video / 30 MB imagen (alineado con validateMediaForIG).
 */
...
export async function POST(req: NextRequest, { params }: Params) {
  ...
  if (insertError || !row) {
    return NextResponse.json({ error: `Error al registrar archivo: ${insertError?.message ?? 'desconocido'}` }, { status: 500 });
  }
