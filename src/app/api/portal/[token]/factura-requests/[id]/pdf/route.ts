...
export async function GET(_req: NextRequest, ctx: { params: Promise<{ token: string; id: string }> }) {
  ...
  // Fetch request and verify PDF path exists
  ...
  if (!fr?.pdf_storage_path) return NextResponse.json({ error: 'not found' }, { status: 404 });

  // Generate signed URL — TTL 5 min (300 s)
  const { data: signed } = await supabase.storage
    .from('cfdi')
    .createSignedUrl(fr.pdf_storage_path, 300);
