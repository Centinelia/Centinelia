    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div className="flex flex-wrap gap-3 items-center">
          ...
          <label className="flex items-center gap-2 text-[13px]">
            <span style={{ color: '#6B6480' }}>Canal</span>
        ...
        <table className="w-full text-[13px]">
          <thead style={{ background: '#FAFAFB' }}>
            <tr>
              ...
              <th className="text-left px-4 py-3 text-[11px] uppercase tracking-wider font-medium" style={{ color: '#6B6480' }}>Canal</th>
              <th className="text-right px-4 py-3 text-[11px] uppercase tracking-wider font-medium" style={{ color: '#6B6480' }}>Llamadas</th>
              <th className="text-right px-4 py-3 text-[11px] uppercase tracking-wider font-medium" style={{ color: '#6B6480' }}>OK rate</th>
              <th className="text-right px-4 py-3 text-[11px] uppercase tracking-wider font-medium" style={{ color: '#6B6480' }}>p50</th>
              <th className="text-right px-4 py-3 text-[11px] uppercase tracking-wider font-medium" style={{ color: '#6B6480' }}>p95</th>
              <th className="text-right px-4 py-3 text-[11px] uppercase tracking-wider font-medium" style={{ color: '#6B6480' }}>Errores</th>
              <th className="text-left px-4 py-3 text-[11px] uppercase tracking-wider font-medium" style={{ color: '#6B6480' }}>Último error</th>
            </tr>
          </thead>
          <tbody>
            {rows.map((r, i) => {
              ...
              return (
                <tr key={`${r.tool_name}::${r.channel}`} className="transition-colors hover:bg-gray-50" style={{ borderTop: i > 0 ? '1px solid #F5F0FF' : undefined }}>
                  <td className="px-4 py-2.5 font-mono" style={{ color: '#1A0A3B' }}>{r.tool_name}</td>
                  <td className="px-4 py-2.5">
                    <ChannelPill channel={r.channel} />
                  </td>
                  <td className="px-4 py-2.5 text-right tabular-nums" style={{ color: '#1A0A3B' }}>{r.calls.toLocaleString()}</td>
                  <td className="px-4 py-2.5 text-right tabular-nums font-medium" style={{ color: okLow ? '#B91C1C' : '#047857' }}>
                    {fmtPct(r.ok_rate)}
                  </td>
                  <td className="px-4 py-2.5 text-right tabular-nums" style={{ color: '#6B6480' }}>{fmtMs(r.latency_p50)}</td>
                  <td className="px-4 py-2.5 text-right tabular-nums font-medium" style={{ color: p95Bad ? '#B91C1C' : '#4A3B6B' }}>
                    {fmtMs(r.latency_p95)}
                  </td>
                  <td className="px-4 py-2.5 text-right tabular-nums" style={{ color: r.err_count > 0 ? '#B91C1C' : '#9B8FB5' }}>
                    {r.err_count}
                  </td>
                  <td className="px-4 py-2.5 text-[12px] font-mono truncate" style={{ color: '#9B8FB5', maxWidth: '280px' }}>{trunc(r.last_error)}</td>
                </tr>
              );
            })}
            {rows.length === 0 && !pending && (
              <tr>
                <td colSpan={8} className="text-center px-4 py-10">
                  <Zap size={20} style={{ color: '#B9B0CF', margin: '0 auto 8px' }} />
                  <p className="text-[13px]" style={{ color: '#6B6480' }}>Sin datos en la ventana seleccionada.</p>
                  <p className="text-[12px] mt-1" style={{ color: '#9B8FB5' }}>Cambia la ventana o el canal, o espera actividad.</p>
                </td>
              </tr>
            )}
          </tbody>
...
function ChannelPill({ channel }: { channel: string }) {
  ...
  const c = colors[channel] ?? { bg: '#F5F0FF', fg: '#4A3B6B', border: '#E8E3F5' };
  return (
    <span
