...
const OUTCOMES: { value: string; label: string; color?: string }[] = [
  { value: '',                     label: 'Todos' },
  { value: 'lead_created',         label: 'Lead',         color: '#10B981' },
...
function AgentCombobox({
  ...
  if (selectedAgent) {
    return (
      <div
        className="flex items-center gap-2 px-3 py-2 rounded-lg text-[13px] min-w-0"
        style={{ background: '#F5F0FF', border: '1px solid #DDD1FF' }}
      >
        <span className="flex-1 truncate font-medium" style={{ color: '#6C3BFF' }}>
...
function FlatCallRow({ call, agentName, timezone }: { call: VoiceCall; agentName: string; timezone: string }) {
  ...
  return (
    ...
    >
      {/* Row header */}
      <div
        className={`flex items-center gap-3 px-4 py-3 transition-colors hover:bg-gray-50 ${hasDetails ? 'cursor-pointer select-none' : ''}`}
